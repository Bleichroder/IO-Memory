					   ...
					   IN PIRP Irp)
{
	...
	struct			 stREG stCpuId,strdmsr,stwrmsr;
	...
	case IOCTL_WINIO_READMSR:
			OutputDebugString("IOCTL_WINIO_READMSR:%d",dwInputBufferLength);
			if(dwInputBufferLength ==4*4)
			{
				memcpy(&strdmsr,pvIOBuffer,dwInputBufferLength);
				__asm
				{
						mov ecx,strdmsr.ECX
						rdmsr
				...
				memcpy (pvIOBuffer, &strdmsr, dwOutputBufferLength);
				Irp->IoStatus.Information = dwOutputBufferLength;
				Irp->IoStatus.Status =STATUS_SUCCESS;
			}
		break;
	case IOCTL_WINIO_WRITEMSR:
			OutputDebugString("IOCTL_WINIO_WRITEMSR:%d",dwInputBufferLength);
			if(dwInputBufferLength ==4*4)
			{
				memcpy(&stwrmsr,pvIOBuffer,dwInputBufferLength);
				__asm
				{
						mov ecx,stwrmsr.ECX
						mov eax,stwrmsr.EAX
