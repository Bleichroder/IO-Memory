...
bool GetDriverPath()
{
	...
	{
		MessageBox(NULL, L"GetModuleFileName fail", L"error", MB_OK);
		return false;
	}
...
bool __stdcall InitializeWinIo()
{
	...
	{		
		...
		if (!bResult)
		{
			MessageBox(NULL, szWinIoDriverPath/*L"InstallWinIoDriver fail"*/, L"error", MB_OK);
			return false;
		}
		...
		if (!bResult)
		{
			MessageBox(NULL, L"StartWinIoDriver fail", L"error", MB_OK);
			return false;
		}
		...
		if (hDriver == INVALID_HANDLE_VALUE)
		{
			MessageBox(NULL, L"CreateFile fail", L"error", MB_OK);
			return false;
		}
	}
	...
	if (!g_Is64BitOS)
	{
		...
		{
			MessageBox(NULL, L"DeviceIoControl fail", L"error", MB_OK);
			return false;
		}
	}
